...
static const camera_metadata_t * camera3_fixup_construct_default_request_settings(android::CameraMetadata metadata, int type)
{
    ...
    uint8_t videoStabilizationMode[1] = {ANDROID_CONTROL_VIDEO_STABILIZATION_MODE_OFF};
    metadata.update(ANDROID_CONTROL_VIDEO_STABILIZATION_MODE, videoStabilizationMode, 1);

    uint8_t afState[1] = {ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE};
    metadata.update(ANDROID_CONTROL_AF_MODE, afState, 1);

    uint8_t afTriggerUpdate[1] = {ANDROID_CONTROL_AF_TRIGGER_CANCEL};
    metadata.update(ANDROID_CONTROL_AF_TRIGGER, afTriggerUpdate, 1);
...
static int camera3_process_capture_request(const camera3_device_t *device, camera3_capture_request_t *request)
{
    ...
    camera_metadata_entry afMode = metadata.find(ANDROID_CONTROL_AF_MODE);
    if (afMode.count > 0) {

        ALOGI("%s: ANDROID_CONTROL_AF_MODE %d ", __FUNCTION__, afMode.data.u8[0]);
    }

    camera_metadata_entry afTrigger = metadata.find(ANDROID_CONTROL_AF_TRIGGER);
    if (afTrigger.count > 0) {
        ALOGI("%s: ANDROID_CONTROL_AF_TRIGGER %d ", __FUNCTION__, afTrigger.data.u8[0]);
    }
    
    camera_metadata_entry afState = metadata.find(ANDROID_CONTROL_AF_STATE);
    if (afState.count > 0) {
        ALOGI("%s: ANDROID_CONTROL_AF_STATE %d ", __FUNCTION__, afState.data.u8[0]);
    }

    if (afMode.count > 0 && afTrigger.count > 0) {
        if (run_count == 0) {
            ALOGI("Setting afcancel on first run");
            run_count++;
            uint8_t afTriggerUpdate[1] = {ANDROID_CONTROL_AF_TRIGGER_CANCEL};
            metadata.update(ANDROID_CONTROL_AF_TRIGGER, afTriggerUpdate, 1);
        }
    ...
    if (afMode.count > 0 && afTrigger.count > 0) {
        if (afMode.data.u8[0] == ANDROID_CONTROL_AF_MODE_AUTO && mode_set == 0) {
            ALOGI("Setting afcancel");
            mode_set++;
            run_count++;
            uint8_t afTriggerUpdate[1] = {ANDROID_CONTROL_AF_TRIGGER_CANCEL};
            metadata.update(ANDROID_CONTROL_AF_TRIGGER, afTriggerUpdate, 1);
        } else if (afMode.data.u8[0] == ANDROID_CONTROL_AF_MODE_AUTO && afTrigger.data.u8[0] == ANDROID_CONTROL_AF_TRIGGER_IDLE && mode_continuous_set == 0) {
            ALOGI("Setting afcancel continuous");
            mode_continuous_set++;
            uint8_t afTriggerUpdate[1] = {ANDROID_CONTROL_AF_TRIGGER_START};
            metadata.update(ANDROID_CONTROL_AF_TRIGGER, afTriggerUpdate, 1);
            uint8_t afModeUpdate[1] = {ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE};
            metadata.update(ANDROID_CONTROL_AF_MODE, afModeUpdate, 1);
        }
    }
        ...
        hw_device_t **device)
{
    ...
    if (name != NULL) {
        ...
        ALOGI("%s: got vendor camera device 0x%08X", __FUNCTION__, (uintptr_t)(camera3_device->vendor));
