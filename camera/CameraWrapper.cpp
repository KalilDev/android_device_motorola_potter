                ...
                hw_device_t** device)
{
    ...
    ALOGI("%s: property persist.camera.HAL3.enabled is set to %d", __FUNCTION__, isHAL3Enabled);
    ...
    if (name != NULL) {
        ...
        if (isHAL3Enabled) {
            ALOGI("%s: using HAL3", __FUNCTION__);
            rv = camera3_device_open(module, name, device);
        } else {
            ALOGI("%s: Only HAL3 is supported", __FUNCTION__);
            return -EINVAL;
        }
    }
